...
app.post("/", async (req, res) => {
  ...
  try {
    ...
    let isReferral =
      webhook.data.purchase?.origin?.src?.includes("ref-vid-") ||
      webhook.data.purchase?.origin?.src?.includes("ref-pay-");

    let referrer;
    if (isReferral) {
      referrer = webhook.data.purchase.origin.src;
    } else {
      referrer = false;
    }
    ...
    if (webhook.event === "PURCHASE_APPROVED") {
      let userRec;
      try {
        userRec = db.collection("users").doc;
      } catch (error) {
        console.log(error);
      }
...
exports.createInDatabase = functions.https.onRequest(app);
